...
Picker на GitHub Pages шлёт `chrome.runtime.sendMessage(extId, …)`.
`extId` должен совпадать с ID **установленного** unpacked/CWS расширения.

- Расширение из Settings/Popup передаёт `?ext=<runtime.id>` автоматически
  (см. `src/js/picker-url.js`).
- При ручном открытии URL добавьте свой ID:
  `…/picker/?intent=set-root&ext=<ID из chrome://extensions>`.
- Hardcoded fallback в `picker/picker.js` (`cmhmigmpifnomnelnecfimefophfgldh`)
  — только для опубликованной CWS-сборки с полем `key`.
